<template>
  <div>
    ...
    <el-table :data="goodsList"
      ...
      <el-table-column sortable label="钢卷号" prop="steelCodes" min-width="152"></el-table-column>
      <el-table-column sortable label="原料规格组成" prop="specAllMa" min-width="120" ></el-table-column>
      <el-table-column sortable label="成品规格组成" prop="spec" min-width="120" ></el-table-column>
      ...
      <el-table-column sortable label="包装日期" prop="createTime" min-width="120"></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 确认选择货物
      confirm() {
        if (this.changeDtList && this.changeDtList.length > 0 && this.changeDtList.length !== this.selectionList.length) { // 换货件数不符合
          this.$alert('您选择的换货件数为' + this.changeDtList.length + '件，当前选中的货物件数为' + this.selectionList.length + '件，换货件数不符合，请重新选择货物', '温馨提示')
          return
        }
